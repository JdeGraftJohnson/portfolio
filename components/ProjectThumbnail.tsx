...
const wrapperStyle: CSSProperties = {
  ...
  backgroundImage:
    "linear-gradient(135deg, rgba(124,58,237,0.18), rgba(49,46,129,0.22))",
...
export function ProjectThumbnail({ id, href, title, external }: Props) {
  ...
  return (
    <a
      ...
      {...externalProps}
    >
      {/* Video lives BENEATH the poster. It's only visible after onPlaying
          flips videoReady → true and fades the poster out. */}
      <video
        ref={videoRef}
        muted
        loop
      ...
      {/* Poster sits ON TOP. Eagerly loaded + sync-decoded so there is no
          intermediate blank state. Fades out only once the video is
      ...
      <img
        ...
        decoding="sync"
        className="group-hover/thumb:scale-110 motion-reduce:!scale-100"
        style={{ ...layerBase, opacity: videoReady ? 0 : 1 }}
      />
      <span
        aria-hidden
